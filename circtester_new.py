...
class CircuitSimulator:
    ...
    @staticmethod
    def get_result_rho_clifford(circ, number_of_qubits, keep_qubits):
        ...
        simulator_noncliff=cirq.DensityMatrixSimulator(dtype=num_precision)
        simulator_cliff=cirq.CliffordSimulator()
        print("simulating....")
        zero_state = np.array([[1, 0], [0, 0]], dtype=num_precision)
        initial_state = zero_state
        # print(circ)
        # circ=remove_idenities(circ)
        print(circ[0])

        print(simulator_cliff.is_supported_operation(circ[4:]))
        print(circ)
        print(circ[4:])
        ...
        # trial_result=simulator_noncliff.simulate(circ, initial_state=initial_state)
        trial_result = simulator_cliff.simulate(circ[4:])
        rho=trial_result.final_state
        print(rho)
        print(type(rho))
        print(type(rho.state_vector()))
        print(np.outer(rho.state_vector(), rho.state_vector()))
        qubits=list(circ.all_qubits())
        print(qubits.sort(key=lambda x: x.name))
        print(qubits)
        projector0_channel=cirq.KrausChannel(
                kraus_ops=(np.array([[1,0],[0,0]]),),
                validate=False
            )
        rho.apply_measurement(cirq.measure(qubits[-1]), {"p0": np.array([[1,0],[0,0]]), "p1": np.array([[0,0],[0,1]])}, np.random.RandomState(),  collapse_state_vector=True)
        print(rho)
        rho=np.outer(rho.state_vector(), rho.state_vector())
